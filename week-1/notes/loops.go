...
func main() {
  ...
  for i < 5 {
    ...
    // If we don't change the value of i, i will _always_ be 0, thus always < 5
    // so this code will run indefinitely and probably crash your application
  }

  // 3:
  // For each byte in the sequence, give back the position (i) and the value
  for i, val := range mystring {
    // fmt.Sprint() converts any interface into a string
